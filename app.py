...
# Models
IMAGE_MODEL = GenerativeModel("gemini-2.5-flash-image-preview")
TEXT_MODEL = GenerativeModel("gemini-2.0-flash")

# ---------------- STREAMLIT UI ----------------
st.set_page_config(page_title="AI Image Generator + Editor", layout="wide")
st.title("🖼️ AI Image Generator + Editor (with Clean Text Overlay)")
...
# ---------------- Prompt Templates ----------------
# (Your original PROMPT_TEMPLATES dict goes here, unchanged)
PROMPT_TEMPLATES = {
    "Marketing": """
...
# (Your original STYLE_DESCRIPTIONS dict goes here, unchanged)
STYLE_DESCRIPTIONS = {
...
def overlay_text_on_image(image_bytes, text_blocks, font_path="arial.ttf", font_size=28, fill=(0,0,0)):
    """
    Overlay clean text on image.
    text_blocks = [("Your text", (x,y)), ...]
    """
    ...
    for text, position in text_blocks:
            ...
            with st.spinner("Generating images..."):
                ...
                if generated_raws:
                    for idx, img_bytes in enumerate(generated_raws):
                        ...
                        # Example flyer text overlay (replace with dynamic inputs later)
                        text_blocks = [
                            ("Why Greddy's?", (50, 50)),
                            ("Comprehensive benefits & competitive compensation", (50, 120)),
                            ("Accelerated career growth", (50, 160)),
                            ("Open Positions:", (50, 240)),
                            ("Recent Graduates – All Disciplines", (50, 280)),
                            ("Campus Visit: Oct 26, 2024 | 10AM – 3PM", (50, 360))
                        ]

                        final_img_bytes = overlay_text_on_image(img_bytes, text_blocks, font_size=26, fill=(10,10,10))

                        st.session_state.generated_images.append({"filename": filename, "content": final_img_bytes})

                        st.image(Image.open(BytesIO(final_img_bytes)), caption=filename, use_column_width=True)
                        st.download_button("⬇️ Download", data=final_img_bytes, file_name=filename, mime="image/png", key=f"dl_{idx}")
        ...
        if not base_image or not raw_prompt_edit.strip():
            ...
            with st.spinner("Editing image..."):
                ...
                for i in range(num_edit_images):
                    out_bytes = run_edit_flow(enhanced_prompt, base_image, f"edit_{i}")
                    if out_bytes:
                        text_blocks = [
                            ("Edited Flyer", (50, 50)),
                            ("Your clean text goes here", (50, 120))
                        ]
                        final_edit_bytes = overlay_text_on_image(out_bytes, text_blocks, font_size=28, fill=(0,0,0))
                        edited_versions.append(final_edit_bytes)
